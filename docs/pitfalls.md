...
v1.3.0 の次のリリースから AGP 9 に上げたことで、R8 の最適化リソース削除 (`android.r8.optimizedResourceShrinking`) が既定で有効になっている (Play Console の推奨対応でもある)。コードの到達可能性まで見て削るので従来より多く消える (Google サインインボタン画像、minSdk 24 未満向けの通知テンプレート、androidx.browser の `image_share_filepaths.xml` など、いずれも未使用)。`tools:keep` は従来どおり効くが、**keep 指定の XML ファイル自体 (`raw/firebase_*_keep.xml` 等) は AAB から消えるのが正常**。残っているべきかは XML ファイルの有無ではなく、keep 対象のリソース名 (`google_app_id` 等) が `base/resources.pb` に残っているかで判断する。

同じく AGP 9 で `android.r8.strictFullModeForKeepRules` も既定 ON になった (`-keep class X` だけではデフォルトコンストラクタが暗黙に残らない)。移行時に OFF でもビルドして mapping を比較し、差分は呼び出し元へインライン化されたコンストラクタ (`GeneratedPluginRegistrant` 内のプラグイン生成、`*Parcelizer` 内の `IconCompat` 等) だけで、リフレクション専用のものは消えていないことを確認済み。プラグインの追加・更新後に **release でだけ** `NoSuchMethodException: <init>` / `InstantiationException` が出たらこれを疑い、該当クラスに `-keep class X { <init>(); }` を足す。
