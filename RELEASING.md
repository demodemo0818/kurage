...
## リリース手順 (Google Play クローズドテスト / AAB)

**現在の Android 配布の主経路**。Google Play のクローズドテストへ **AAB (App Bundle)** をアップロードする。APK ではアップロードできない。
...
7. **Play Console での操作はユーザー側**: クローズドテスト → 新しいリリースを作成 →
...
> **正式版リリース後に再検討するため一時停止中**。当面 Android は上記 Google Play (AAB) のみ。
> 再開時はこの手順で APK を作り `dist/*.apk` に置く。

クローズドアルファ / ベータ期間は LINE / Discord などで APK を直接共有する想定。
...
(コミット対象。`/dist/` は `.gitignore` 済みで別 PC に同期されないため、
`release_notes/` に置いて versioned に保つ)。

- テスター視点で読みやすい日本語にまとめる (生のコミット件名の羅列にしない)。
  「✨ 新機能 / 🛠 改善・修正 / 📝 既知の注意点」等のカテゴリに整理。
- 元ネタは `git log <前回タグ>..HEAD`。ただし **前回タグをビルドしたが配布せず
  スキップした場合は、最後に "配布した" バージョンまで遡って全部込みで書く**
  (例: 0.8.0 をスキップして 0.8.1 を配布 → 0.8.1 のノートは v0.7.0-alpha からの
  全変更をカバー)。
- 冒頭に対象バージョン・配布物 (Android APK / Windows zip / Web) とアルファ版の
  注意を明記。Windows zip は未署名なので初回 SmartScreen 回避手順 (「詳細情報」→
  「実行」) を 1 行添える。
...
- アルファ / ベータ版である旨と、破壊的変更 (再ログイン要求 / データ消去) が
  起こりうることを事前に説明する。
- バグ報告先 (連絡先 / Issue リンク) を明示。
- インストール手順 (提供元不明アプリの許可) を最初の 1 回だけ案内。
- バージョン番号の読み方: PATCH 上がり = バグ修正のみ、MINOR 上がり =
  機能追加または仕様変更 (前バージョンと挙動が変わるかも)、というガイド。
