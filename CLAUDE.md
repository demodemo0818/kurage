...
- **AGP 9.1.0 / Kotlin 2.4.0 / Gradle wrapper 9.3.1**（[android/settings.gradle.kts](android/settings.gradle.kts)、[gradle-wrapper.properties](android/gradle/wrapper/gradle-wrapper.properties)）。Flutter 3.47.5 のテンプレートと同じ組み合わせ。Gradle 9 / AGP 9 は **JDK 17 以上必須**。Flutter 3.47 の Gradle プラグインは Gradle < 9.1 / Kotlin < 2.3.20 / AGP < 9.0.1 で「まもなくサポート終了」の警告を出し、**Gradle < 8.14 / Kotlin < 2.2.20 / AGP < 8.11.1 だと `Failed to apply plugin 'dev.flutter.flutter-gradle-plugin'` でビルドを止める** (flutter_tools は「AGP 9 の新 DSL」の案内を出すが、真因は Gradle ログの `DependencyValidationException`)。アプリの `build.gradle.kts` は組み込み Kotlin 移行済み (Kotlin プラグインを apply せず、`kotlin { compilerOptions }` で jvmTarget 指定)。ただし `android/gradle.properties` の `android.builtInKotlin=false` / `android.newDsl=false` は AGP 9 既定からのオプトアウトで、まだ Kotlin Gradle Plugin を自前 apply するプラグインが多いこと・Flutter の Gradle プラグイン自体が旧 DSL を使うことから外せない (AGP 10 で廃止予定。Flutter とプラグインの対応を待つ)。AGP 9 で R8 の既定が変わった点 (最適化リソース削除 / strict full mode) は [docs/pitfalls.md](docs/pitfalls.md) 参照。
